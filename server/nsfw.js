...
router.get('/nhentaipdf', async (req, res) => {
	var code = req.query.code
	var nhres = `https://new-apii.herokuapp.com/nsfw/nhcode?query=${code}`
	var nhread = `https://new-apii.herokuapp.com/nsfw/nhread?query=${code}`
	res.json({
		pdf: nhres,
		read: nhread,
...
router.get('/nhcode', async (req, res) => {
        try {
	var query = req.query.query
	let data = await axios.get('https://new-apii.herokuapp.com/nsfw/nHentai?code='+query)
